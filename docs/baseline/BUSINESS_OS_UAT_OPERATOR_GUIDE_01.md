...
4. nhân viên phân công 5 slot trong checklist, không để Codex tự chọn khách hàng.
...
2. Đối chiếu snapshot preflight để biết slot nào đã có coverage và slot nào cần hồ sơ mới.
3. Nhân viên chịu trách nhiệm xác nhận từng Lead/Project được phép dùng.
4. Chạy lần lượt từng hồ sơ; không chạy đồng thời cả 5 hồ sơ trong lần UAT đầu.
5. Mỗi bước phải lưu trạng thái `PASS`, `FAIL` hoặc `BLOCKED` cùng bằng chứng.
6. Gặp blocker mức cao/nghiêm trọng thì dừng hồ sơ, không cố chuyển bước bằng sửa trực tiếp database.
