...
const UPDATE_PENDING_CODE_KEY = '@sx_update_pending_code';
const UPDATE_PENDING_VERSION_KEY = '@sx_update_pending_version';
const UPDATE_DISMISSED_CODE_KEY = '@sx_update_dismissed_code';
const UPDATE_DISMISSED_VERSION_KEY = '@sx_update_dismissed_version';
...
export async function openDownloadedApk(
  ...
  if (!canInstall) {
    const err = new Error(
      'Cần bật "Cho phép cài đặt ứng dụng không rõ nguồn" cho Quản lý sản xuất.',
