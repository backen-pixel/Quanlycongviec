...
export default function ProfileScreen() {
  ...
  const companyName = useMemo(() => {
    if (!user?.company_id) return isSystemAdmin(user) ? 'Toàn hệ thống' : 'Chưa gán xưởng';
    return companies.find((c) => String(c.id) === String(user.company_id))?.name || 'Đã gán xưởng';
  }, [user, companies]);
  ...
  const quickItems: QuickItem[] = [
    {
      ...
      icon: 'qr-code',
      color: '#8B5CF6',
      onPress: () => Alert.alert('Quét mã QR', 'Tính năng quét mã sắp có trên app xưởng.'),
    },
    {
      key: 'camera',
      label: 'Chụp ảnh',
    ...
    },
    {
      ...
      icon: 'document-text',
      color: '#22C55E',
      onPress: () => DeviceEventEmitter.emit(SX_OPEN_CREATE_DEAL),
    },
    {
      key: 'note',
      label: 'Ghi chú',
        ...
        <View style={styles.userCard}>
          ...
          <View style={styles.userMetaRow}>
            <View style={styles.userMetaItem}>
              <Text style={styles.userMetaLabel}>Xưởng</Text>
