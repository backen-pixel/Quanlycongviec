...
export default function OverdueProjectsScreen() {
  ...
  const filtered = useMemo(() => {
    ...
    const list = [...projects].sort((a, b) => {
      const da = new Date(a.delivery_date || a.production_deadline || a.deadline || 0).getTime();
      const db = new Date(b.delivery_date || b.production_deadline || b.deadline || 0).getTime();
      return da - db;
    });
    if (!q) return list;
        ...
        <View style={styles.cardTop}>
          ...
          <View style={{ flex: 1, minWidth: 0 }}>
            ...
            <Text style={styles.meta} numberOfLines={1}>
              {item.customer_name || '—'}
              {item.production_person_name ? ` · ${item.production_person_name}` : ''}
            </Text>
          </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <Text style={styles.empty}>
              {search ? 'Không tìm thấy deal phù hợp' : 'Không có deal quá hạn'}
            </Text>
          }
        />
      )}
