 ...
 * Cần file sx-mobile/google-services.json (package vn.tubeppro.sxmobile).
 */
const fs = require('fs');
const path = require('path');
const { withDangerousMod, withAppBuildGradle } = require('@expo/config-plugins');

const PACKAGE = 'vn.tubeppro.sxmobile';
  ...
  if (!src) {
    console.warn(
      `[withGoogleServices] Chưa có ${SOURCE_NAME} cho ${PACKAGE}. ` +
        'Firebase Console → thêm app Android vn.tubeppro.sxmobile → tải google-services.json vào sx-mobile/',
    );
    return false;
  }
