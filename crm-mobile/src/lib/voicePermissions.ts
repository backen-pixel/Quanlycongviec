...
/** Giống `perm_title` — Voice Sync (`strings.xml`). */
export const VOICE_PERM_TITLE = 'Quyền ghi âm & cuộc gọi';

/**
 * Giống `perm_message` — TuBep Voice Sync (`strings.xml`), một hộp thoại + một nút «Cấp quyền».
 */
...
/** Giống `permissionsNeeded()` trong Voice Sync `MainActivity.kt`. */
...
/**
 * Xin quyền ngay — không qua hộp thoại (nhanh nhất, tương đương đã bấm «Cấp quyền» trên Voice Sync).
 */
...
/**
 * Giống Voice Sync: một `AlertDialog` — chỉ nút «Cấp quyền» → mở hệ thống xin quyền (không có «Để sau»).
 */
...
/** @deprecated Dùng `showVoicePermissionDialogThenRequest` hoặc `requestVoicePermissionsQuick`. */
export const explainThenRequestVoicePermissions = showVoicePermissionDialogThenRequest;
