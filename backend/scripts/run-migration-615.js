 ...
 * Usage: node scripts/run-migration-615.js
 */
const fs = require('fs');
...
const SQL = fs.readFileSync(
  path.join(__dirname, '..', '..', 'database', '615_hcb_tubep_dong_goi_group_key.sql'),
  'utf8',
).split('-- HOÀN TÁC:')[0];
...
const VERIFY_SQL = `
SELECT wt.name AS loai, pps.name, pps.group_key
...
WHERE (c.short_name = 'HCB' OR c.name ILIKE '%Hucabi%')
  AND pps.group_key = 'dong_goi'
ORDER BY wt.name, pps.order_index;
`;
