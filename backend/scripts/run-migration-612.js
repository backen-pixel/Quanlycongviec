/**
 * 612: HCB Cửa/Cánh kính — cột lớn Đóng gói.
 * Usage: node scripts/run-migration-612.js
 */
const fs = require('fs');
...
const SQL = fs.readFileSync(
  path.join(__dirname, '..', '..', 'database', '612_hcb_dong_goi_group_key.sql'),
  'utf8',
).split('-- HOÀN TÁC:')[0];
...
const VERIFY_SQL = `
SELECT wt.name AS loai, pps.name, pps.group_key, pps.is_packaging_done
...
WHERE c.short_name = 'HCB'
  ...
  AND lower(trim(pps.name)) = 'vệ sinh đóng gói'
ORDER BY wt.name;
`;
