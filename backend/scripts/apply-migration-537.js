/**
 * Apply database/537_workflow_flow_node_kind.sql via Management API.
 * Usage: node scripts/apply-migration-537.js
 *        node scripts/apply-migration-537.js --backup
 */
...
async function main() {
  ...
  const query = fs.readFileSync(
    path.join(__dirname, '../../database/537_workflow_flow_node_kind.sql'),
    'utf8',
  );
...
WHERE table_name = 'workflow_flow_steps'
  AND column_name IN ('node_kind', 'node_config');
`;
