/**
 * Apply database/543_project_delivery_workflow_stages.sql via Management API.
 * Usage: node scripts/apply-migration-543.js
 *        node scripts/apply-migration-543.js --backup
 */
...
async function main() {
  ...
  const query = fs.readFileSync(
    path.join(__dirname, '../../database/543_project_delivery_workflow_stages.sql'),
    'utf8',
  );
  ...
  const verify = `
SELECT slug, name, order_index, is_active
FROM workflow_stages
WHERE company_id IS NULL
  AND slug IN (
    'order','design','approve','measure','production',
    'materials','delivery','installation','acceptance','warranty',
    'consulting','shipping','customer-care'
  )
