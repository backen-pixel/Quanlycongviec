/**
 * 610: HCB Cánh kính/Cửa — Tiếp nhận không chặn kéo cột.
 * Usage: node scripts/run-migration-610.js
 */
const fs = require('fs');
...
const SQL = fs.readFileSync(
  path.join(__dirname, '..', '..', 'database', '610_hcb_canh_kinh_tiep_nhan_khong_chan_keo.sql'),
  'utf8',
);
...
const VERIFY_SQL = `
SELECT wpt.name AS loai, pps.name AS cot,
  (SELECT count(*) FROM crm_tasks ct
     WHERE ct.production_pipeline_stage_id = pps.id AND ct.blocks_stage_advance = true
