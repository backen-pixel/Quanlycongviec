/**
 * Apply database/541_sales_company_workshop_module_scope.sql via Management API.
 * Usage: node scripts/apply-migration-541.js
 *        node scripts/apply-migration-541.js --backup
 */
...
async function main() {
  ...
  const query = fs.readFileSync(
    path.join(__dirname, '../../database/541_sales_company_workshop_module_scope.sql'),
    'utf8',
  );
  ...
  const verify = `
SELECT u.role::text AS role, c.name AS company, COUNT(*) AS n
FROM users u
LEFT JOIN companies c ON c.id = u.company_id
WHERE lower(u.role::text) IN ('sales', 'sales_admin')
  AND COALESCE(u.is_active, true) = true
GROUP BY u.role, c.name
ORDER BY u.role, n DESC;

SELECT u.email, u.full_name, u.role::text, c.name AS company
FROM users u
LEFT JOIN companies c ON c.id = u.company_id
WHERE lower(u.role::text) IN ('sales', 'sales_admin')
  AND COALESCE(u.is_active, true) = true
