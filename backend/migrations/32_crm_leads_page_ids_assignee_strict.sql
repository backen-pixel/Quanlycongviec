-- Thêm p_assigned_strict: true = chỉ khớp crm_leads.assigned_to (phụ trách deal), không OR lead_owner_id.
-- Dùng cho NV không thuộc admin/manager/director khi xem pipeline Deal.
...
COMMENT ON FUNCTION public.crm_leads_page_ids IS
  'Trả về { total, ids } cho GET /api/crm/leads: phân trang + lọc SĐT. p_assigned_strict=true: chỉ assigned_to.';
