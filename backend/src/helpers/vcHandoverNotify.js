...
async function notifyVcHandoverFromSx(req, {
  ...
  try {
    ...
    const title = manual
      ? '🚚 Chờ vận chuyển · Bàn giao từ Xưởng'
      : '🚚 Chờ vận chuyển · Deal mới từ Xưởng';
    const message = manual
      ? `Dự án ${code} đã bàn giao sang Vận chuyển & Lắp đặt — đang ở cột Chờ vận chuyển`
      : `Dự án ${code} đã hoàn thành sản xuất, vừa vào cột Chờ vận chuyển`;
