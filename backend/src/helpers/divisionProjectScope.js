...
/** Giai đoạn chưa sản xuất xong — map vào ô «planning» cũ trên dashboard Khối. */
const PLANNING_PROJECT_STATUSES = ['consulting', 'designing', 'quoting'];
/** Đang sản xuất / giao / lắp — map vào ô «in_progress» cũ. */
const IN_PROGRESS_PROJECT_STATUSES = ['contract_signed', 'producing', 'shipping', 'installing'];
 ...
 */
async function loadDivisionProjects(divisionId, columns) {
  const ids = await resolveDivisionProjectIds(divisionId);
  if (!ids.length) return [];
  return fetchAllByIds({ table: 'projects', columns, key: 'id', ids });
}
 ...
 * @returns {Promise<object[]>}
 */
async function loadTasksForProjectIds(projectIds, columns) {
  const ids = [...new Set((projectIds || []).filter(Boolean).map(String))];
  if (!ids.length) return [];
  return fetchAllByIds({ table: 'tasks', columns, key: 'project_id', ids });
}
