...
function parseMaxContactsLinkCleanup(raw) {
  ...
  if (!Number.isFinite(n) || n < 1) return DEFAULT_MAX_CONTACTS_LINK_CLEANUP;
  return Math.min(MAX_CONTACTS_LINK_CLEANUP_CAP, n);
}
...
const FB_FETCH_PROFILE_PIC = ['1', 'true', 'yes', 'on'].includes(
  String(process.env.FB_FETCH_PROFILE_PIC || '').toLowerCase(),
);
 ...
 *   `.in(...)`        gãy khi vượt 643 id
 *   chuỗi `or(...)`   gãy khi vượt 556 id
 *   tương ứng URL     khoảng 22.000–24.000 ký tự
 * Vượt nữa thì ĐỨT KẾT NỐI, không phải trả về lỗi tử tế.
 *
 * 300 là ngưỡng cảnh báo mà chính guard đó dùng (SUPABASE_GUARD_IN_WARN), nên
 * lấy luôn 300 để không tự bật cảnh báo của mình. Một trang 1000 dòng thành 4
 * round-trip — rẻ hơn nhiều so với rủi ro đứt kết nối.
 *
 * GHI LẠI MỘT LẦN TÔI SUY LUẬN SAI: trước đó tôi từng hạ số này xuống 200, rồi
 * TỰ HOÀN NGUYÊN về 800 với lý do "800 đã chạy trong production nên là bằng
 * chứng tốt nhất". Suy luận đó sai: tôi lấy số lượt quét của một chỉ mục dùng
 * chung cho nhiều đường làm bằng chứng cho riêng đường này. Số đo thật cho thấy
 * 800 nằm TRÊN ngưỡng gãy 643. Bài học: "chạy được lâu nay" không phải bằng
 * chứng, chỉ số đo mới là.
 */
const IN_CHUNK = 300;
...
/** % tăng/giảm so với kỳ trước. prev=0 mà current>0 thì không có % ý nghĩa -> null ("mới"). */
function analyticsPctChange(curr, prev) {
...
async function runPipelineV2OnePass({
  ...
  for (let i = 0; i < contacts.length; i++) {
    ...
    let phoneAfterCustId = fresh?.customer_id || null;
    if (!phoneAfterCustId && fresh?.lead_id) {
      const { data: ldp } = await supabase.from('crm_leads').select('customer_id').eq('id', fresh.lead_id).maybeSingle();
      phoneAfterCustId = ldp?.customer_id || null;
    }
    let phone = null;
    if (phoneAfterCustId) {
      const { data: cp } = await supabase.from('customers').select('phone').eq('id', phoneAfterCustId).maybeSingle();
      phone = cp?.phone && String(cp.phone).trim() ? String(cp.phone).trim() : null;
    }
...
/** Avatar giả (ui-avatars) hoặc chưa có — chỉ dùng khi FB_FETCH_PROFILE_PIC bật. */
function isPlaceholderFacebookAvatar(url) {
  if (!FB_FETCH_PROFILE_PIC) return false;
  if (!url) return true;
...
async function fetchFacebookUserProfileFromGraph(psid, token) {
  if (!FB_FETCH_PROFILE_PIC || !psid || !token) return null;
  const uid = encodeURIComponent(String(psid).trim());
...
/** Resolve tên qua Conversations API; emit socket để UI cập nhật ngay. */
...
// Lấy tên qua Conversations API — không gọi Graph profile_pic (trừ khi FB_FETCH_PROFILE_PIC=1)
async function fetchProfileNameOnlyViaConversations(pageId, psid) {
  try {
    ...
    try {
      ...
      if (participant?.name && participant.name !== 'Facebook User') {
        ...
        return { name: participant.name, source: 'participants' };
      }
    } catch (_) { /* fallthrough */ }
    ...
    await logFetchResult(pageId, psid, 'success_messages', { name: userMsg.from.name });
    return { name: userMsg.from.name, source: 'messages' };
  } catch (e) {
    console.warn('[FB] fetchProfileNameOnlyViaConversations error:', e.message);
    return null;
  }
}

async function fetchProfileViaConversations(pageId, psid, { logContext = null } = {}) {
  if (!FB_FETCH_PROFILE_PIC) {
    return fetchProfileNameOnlyViaConversations(pageId, psid);
  }
...
/** Lead đã gắn KH và KH đã có SĐT → không quét lại tin nhắn (chỉ coi SĐT khách hàng, không dùng SĐT cột contact FB / mô tả). */
...
async function handleMessagingInner(pageId, event, io, partnerPsid) {
  ...
  if (event.message) {
    ...
    if (!isEcho) {
      ...
      if (!contact.lead_id) {
        ...
        if (hadLeadBefore && !autoLeadCfg.recreate_deleted_leads) {
          ...
          switch (autoLeadCfg.trigger) {
            ...
            case 'has_phone':
              // Chỉ tạo khi có SĐT trong tin nhắn (không dùng cột facebook_contacts.phone)
              if (extractedPhone) {
                shouldCreate = true;
      ...
      let linkedCustPhoneDigits = 0;
      if (contact.lead_id) {
        const { data: leadRow } = await supabase.from('crm_leads').select('customer_id').eq('id', contact.lead_id).maybeSingle();
        if (leadRow?.customer_id) {
          const { data: custRow } = await supabase.from('customers').select('phone').eq('id', leadRow.customer_id).maybeSingle();
          linkedCustPhoneDigits = _phoneDigitsLen(custRow?.phone);
        }
      }

      // Nếu không tìm thấy SĐT trong tin nhắn hiện tại, quét tin nhắn cũ nếu lead/KH chưa có SĐT chuẩn
      if (!extractedPhone && contact.lead_id && linkedCustPhoneDigits < 9) {
        const { data: oldMsgs } = await supabase.from('facebook_messages')
      ...
      if ((extractedPhone && autoLeadCfg.auto_update_phone) || (extractedAddress && autoLeadCfg.auto_update_address)) {
        if (extractedPhone && autoLeadCfg.auto_update_phone) {
          console.log(`[FB] 📞 Phone from message: ${extractedPhone} (lưu KH, không ghi facebook_contacts.phone)`);
        }
...
async function handleLeadGen(pageId, value) {
  ...
  if (contact) {
    if (email) {
      await supabase.from('facebook_contacts').update({ email, updated_at: new Date().toISOString() }).eq('id', contact.id);
    }
...
async function resolveFacebookPageScope(req, res, opts = {}) {
  ...
  if (forcedLeadCompanyId && String(forcedLeadCompanyId) !== String(cid)) {
    res.status(403).json({ error: 'Không có quyền xem dữ liệu Facebook của deal công ty khác.' });
    return null;
  }
  return {
...
r.get('/contacts', authMiddleware, async (req, res) => {
  try {
    ...
    if (hasDateFilter) {
      ...
      merged = [];
      // 800 id/lô nằm TRÊN ngưỡng gãy đã đo được của `.in(...)` là 643
      // (helpers/supabaseQueryGuard.js). Dùng IN_CHUNK như mọi chỗ khác.
      const CH = IN_CHUNK;
    ...
    result.forEach((c) => {
      const p = c.customer?.phone && String(c.customer.phone).trim() !== '' ? String(c.customer.phone).trim() : null;
      c.display_phone = p;
    });
...
r.post('/contacts/:id/sync-history', authMiddleware, async (req, res) => {
  try {
    ...
    if (!contact.lead_id && synced > 0) {
      // Extract phone/address từ messages vừa sync (không lấy từ cột facebook_contacts.phone)
      let extractedPhone = null;
      let extractedAddress = null;
...
r.get('/contacts/:contactId/messages', authMiddleware, async (req, res) => {
  try {
    const scope = await resolveFacebookPageScope(req, res);
    if (!scope) return;
    const { data: contact, error: cErr } = await supabase.from('facebook_contacts')
      .select('id, page_id').eq('id', req.params.contactId).maybeSingle();
    if (cErr) throw cErr;
    if (!contact) return res.status(404).json({ error: 'Contact not found' });
    if (!contactAllowedByFacebookScope(scope, contact)) {
...
// Get messages by lead_id (deal/lead CRM): scope theo company của deal; tìm contact theo lead_id hoặc customer_id
r.get('/leads/:leadId/messages', authMiddleware, async (req, res) => {
  try {
    const { data: lead, error: leadErr } = await supabase.from('crm_leads')
      .select('id, customer_id, company_id, type')
      .eq('id', req.params.leadId)
      .maybeSingle();
    if (leadErr) throw leadErr;
    if (!lead) return res.status(404).json({ error: 'Không tìm thấy lead/deal' });

    const scopeOpts = lead.company_id ? { leadCompanyId: lead.company_id } : {};
    const scope = await resolveFacebookPageScope(req, res, scopeOpts);
    if (!scope) return;

    const contactSelect = 'id, fb_name, fb_profile_pic, phone, psid, page_id, lead_id, customer_id';
    let contact = null;

    const { data: byLead } = await supabase.from('facebook_contacts')
    ...
    if (!contact && lead.customer_id) {
      const { data: byCust } = await supabase.from('facebook_contacts')
        .select(contactSelect)
    ...
    const { data: fallback } = await supabase.from('facebook_messages')
...
function emptyFacebookAnalyticsPayload() {
  return {
    ...
    comparison: {
      totalContacts: { current: 0, previous: 0, pct: 0 },
      totalMessages: { current: 0, previous: 0, pct: 0 },
      dealCount: { current: 0, previous: 0, pct: 0 },
      hasPhone: { current: 0, previous: 0, pct: 0 },
      hasLead: { current: 0, previous: 0, pct: 0 },
    },
    firstResponseTime: {
      avgMinutes: null,
      medianMinutes: null,
...
r.get('/analytics', authMiddleware, async (req, res) => {
  try {
    ...
    const hasPhonePrev = contactsInPrevPeriod.filter(analyticsContactHasPhone).length;
    const hasLeadPrev = contactsInPrevPeriod.filter(c => c.lead_id).length;

    // Deals — .in() quá dài có thể lỗi; chia batch. Tách hàm để tính lại được cho kỳ trước (so sánh %).
    const DEAL_IN_BATCH = 500;
    async function countDealsForLeadIds(leadIds) {
      let n = 0;
    ...
    const comparison = {
      ...
      totalMessages: { current: messages.length, previous: messagesPrev.length, pct: analyticsPctChange(messages.length, messagesPrev.length) },
      dealCount: { current: dealCount, previous: dealCountPrev, pct: analyticsPctChange(dealCount, dealCountPrev) },
      hasPhone: { current: hasPhone, previous: hasPhonePrev, pct: analyticsPctChange(hasPhone, hasPhonePrev) },
      hasLead: { current: hasLead, previous: hasLeadPrev, pct: analyticsPctChange(hasLead, hasLeadPrev) },
    };
    ...
    (pages || []).forEach((p) => {
      ...
      pageBk[p.page_id] = {
        ...
        has_phone: 0,
        has_lead: 0,
        unattended: unattendedByPage.get(p.page_id) || 0,
      };
    });
    contactsInPeriod.forEach((c) => {
      if (!pageBk[c.page_id]) {
        pageBk[c.page_id] = {
          page_id: c.page_id, page_name: String(c.page_id), contacts: 0, has_phone: 0, has_lead: 0,
          unattended: unattendedByPage.get(c.page_id) || 0,
        };
      }
    ...
    // Avg response time (mọi lượt qua-lại) + Thời gian phản hồi LẦN ĐẦU (first inbound -> first outbound
    // sau đó trong cùng contact) + Hiệu suất theo nhân viên (dựa vào sent_by của tin outbound).
    ...
    const bumpEmployee = (sentBy) => {
      if (!sentBy) return null;
      if (!employeeStats.has(sentBy)) employeeStats.set(sentBy, { messagesSent: 0, contactsHandled: new Set(), rtSum: 0, rtCount: 0 });
      return employeeStats.get(sentBy);
    };

    Object.values(byC).forEach(cm => {
      // cm đã theo thứ tự thời gian tăng dần vì fetchAllAnalyticsMessagesFor... order by created_at asc.
      let firstInboundIdx = -1;
      for (let i = 0; i < cm.length; i++) {
        if (cm[i].direction === 'outbound') {
          const emp = bumpEmployee(cm[i].sent_by);
          if (emp) { emp.messagesSent++; emp.contactsHandled.add(cm[i].contact_id); }
        }
        if (firstInboundIdx === -1 && cm[i].direction === 'inbound') firstInboundIdx = i;
      }
      if (firstInboundIdx !== -1) {
        for (let j = firstInboundIdx + 1; j < cm.length; j++) {
          if (cm[j].direction === 'outbound') {
            const diff = new Date(cm[j].created_at) - new Date(cm[firstInboundIdx].created_at);
            if (diff > 0 && diff < 86400000) firstResponseSamplesMs.push(diff);
            break;
          }
        }
      }
      for (let i = 0; i < cm.length - 1; i++) {
        if (cm[i].direction === 'inbound' && cm[i + 1].direction === 'outbound') {
          ...
          if (diff > 0 && diff < 86400000) {
            totalRT += diff; rtCount++;
            const emp = bumpEmployee(cm[i + 1].sent_by);
            if (emp) { emp.rtSum += diff; emp.rtCount++; }
          }
        }
      }
    });

    const frtCount = firstResponseSamplesMs.length;
    const firstResponseTime = {
      avgMinutes: frtCount ? Math.round(firstResponseSamplesMs.reduce((a, b) => a + b, 0) / frtCount / 60000) : null,
      medianMinutes: frtCount ? Math.round(analyticsMedian(firstResponseSamplesMs) / 60000) : null,
      sampleCount: frtCount,
      slaUnder15m: frtCount ? Math.round(firstResponseSamplesMs.filter(v => v <= 15 * 60000).length / frtCount * 100) : 0,
      slaUnder1h: frtCount ? Math.round(firstResponseSamplesMs.filter(v => v <= 60 * 60000).length / frtCount * 100) : 0,
      slaUnder4h: frtCount ? Math.round(firstResponseSamplesMs.filter(v => v <= 240 * 60000).length / frtCount * 100) : 0,
    };

    const employeeIds = [...employeeStats.keys()];
    let employeeNameMap = new Map();
    if (employeeIds.length) {
      const { data: staffRows } = await supabase.from('users').select('id, full_name').in('id', employeeIds);
      employeeNameMap = new Map((staffRows || []).map((u) => [u.id, u.full_name]));
    }
    const employeePerformance = employeeIds
      .map((id) => {
        const s = employeeStats.get(id);
        return {
          user_id: id,
          name: employeeNameMap.get(id) || 'NV không xác định',
          messages_sent: s.messagesSent,
          contacts_handled: s.contactsHandled.size,
          avg_response_minutes: s.rtCount ? Math.round(s.rtSum / s.rtCount / 60000) : null,
        };
      })
...
r.post('/batch-extract-phones', authMiddleware, async (req, res) => {
  try {
    ...
    // Thứ tự mới→cũ; ưu tiên contact chưa có SĐT trên KH (không dùng cột facebook_contacts.phone).
    contacts.sort((a, b) => {
      const act = activityTimestampMs(b) - activityTimestampMs(a);
      if (act !== 0) return act;
      const score = (c) => {
        ...
        const cust = custFromLead || custFromContact;
        if (_phoneDigitsLen(cust?.phone) < 9) s += 2;
        if (!c.lead_id) s += 0.5;
        return s;
      };
    ...
    for (let i = 0; i < contactsToProcess.length; i++) {
      ...
      // SĐT đồng bộ vào KH / mô tả: chỉ từ tin nhắn (extracted), không fallback cột facebook_contacts.phone
      const effectivePhone = extractedPhone || null;
      const custPhoneOk = _phoneDigitsLen(cust?.phone) >= 9;

      if (!effectivePhone && !extractedAddress && extraPhones.length === 0 && !custPhoneOk) {
        noInfo++;
      ...
      if (lead && contact.lead_id) {
        ...
        let desc = lead.description || '';
        const phoneForDesc = (_phoneDigitsLen(cust?.phone) >= 9 && String(cust.phone).trim()) ? String(cust.phone).trim() : effectivePhone;
        if (phoneForDesc) {
          if (/SĐT:/.test(desc)) {
            const oldMatch = desc.match(/SĐT:\s*(\S*)/);
            if (!oldMatch?.[1] || oldMatch[1] !== phoneForDesc) {
              desc = desc.replace(/SĐT:.*$/m, `SĐT: ${phoneForDesc}`);
            }
          } else {
            desc = `${desc.trimEnd()}\nSĐT: ${phoneForDesc}`.trim();
          }
        }
        if (extractedAddress) {
      ...
      results.push({
        contact_id: contact.id,
        contact: contact.fb_name,
        phone: cust?.phone && String(cust.phone).trim() ? String(cust.phone).trim() : effectivePhone || null,
        address: extractedAddress || cust?.address || lead?.install_address || null,
        extraPhones: extraPhones.length ? extraPhones : undefined,
        status: extractedPhone ? 'updated_phone' : extractedAddress ? 'updated_address' : custPhoneOk ? 'refreshed' : 'refreshed',
      });
      if (io) io.emit('batch_progress', {
        ...
        name: contact.fb_name,
        status: 'found',
        phone: cust?.phone && String(cust.phone).trim() ? String(cust.phone).trim() : extractedPhone || null,
        address: extractedAddress || cust?.address || lead?.install_address || null,
      });
    }
...
async function applyExtractFromDbMessagesForContact(contact, { forceRescanPhones = false } = {}) {
  ...
  if (lead && fresh.lead_id) {
    ...
    let desc = lead.description || '';
    const phoneForDesc = (_phoneDigitsLen(cust?.phone) >= 9 && String(cust.phone).trim())
      ? String(cust.phone).trim()
      : effectivePhone;
    if (phoneForDesc) {
      if (/SĐT:/.test(desc)) {
        const oldMatch = desc.match(/SĐT:\s*(\S*)/);
        if (!oldMatch?.[1] || oldMatch[1] !== phoneForDesc) desc = desc.replace(/SĐT:.*$/m, `SĐT: ${phoneForDesc}`);
      } else {
        desc = `${desc.trimEnd()}\nSĐT: ${phoneForDesc}`.trim();
      }
    }
  ...
  const outPhone = cust?.phone && String(cust.phone).trim() ? String(cust.phone).trim() : effectivePhone || null;
  return {
    outcome: 'updated',
    phone: outPhone,
    address: extractedAddress || cust?.address || lead?.install_address || null,
...
// ── Refresh tên cho contact placeholder (không quét avatar) ──
r.post('/refresh-names', authMiddleware, async (req, res) => {
  try {
    ...
    if (!stuckContacts?.length) {
      return res.json({ updated: 0, message: 'Không có contact nào cần cập nhật tên', log_lines: [] });
    }
    ...
    console.log(`[FB Refresh names] Bắt đầu: ${total} contact`);
    emitRefreshLog({ phase: 'start', total, current: 0, message: `Bắt đầu refresh tên ${total} contact` });
    ...
    for (let i = 0; i < stuckContacts.length; i++) {
      ...
      if (FB_FETCH_PROFILE_PIC && shouldUpdateFacebookProfilePic(c.fb_profile_pic, profile?.profilePic)) {
        upd.fb_profile_pic = profile.profilePic;
        changed = true;
      }
      ...
      emitRefreshLog({
        ...
        message: changed ? `${resolvedName}: cập nhật tên` : `${resolvedName}: không đổi`,
      });
      ...
      if (changed) {
        ...
        console.log(`[FB Refresh names] ${c.psid}: "${c.fb_name}" → "${resolvedName}"`);
      }
    }
    const summary = { updated, total, log_lines: logLines.slice(-50) };
    console.log(`[FB Refresh names] Xong: ${updated}/${total}`);
    if (io) io.emit('batch_done', { type: 'refresh_names', ...summary });
    res.json(summary);
  } catch (e) {
    console.error('[FB Refresh names] Lỗi:', e.message);
    res.status(500).json({ error: e.message });
  }
});
...
async function runRescanPhonesBatch(body, ioRef) {
  ...
  let poolSorted = sortFacebookContactsNewestFirst(contacts);
  const leadIdsRescan = [...new Set(poolSorted.map((c) => c.lead_id).filter(Boolean))];
  const leadMapRescan = {};
  ...
  for (const c of poolSorted) {
    const cid = c.customer_id || (c.lead_id && leadMapRescan[c.lead_id]?.customer_id);
    if (cid) custIdSetRescan.add(cid);
  }
  const custMapRescan = {};
  ...
  function rescanCustId(row) {
    return row.customer_id || (row.lead_id && leadMapRescan[row.lead_id]?.customer_id) || null;
  }
  function rescanCustPhoneDigitsLen(row) {
    const id = rescanCustId(row);
    if (!id) return 0;
    return _phoneDigitsLen(custMapRescan[id]?.phone);
  }
  if (mode === 'with_phone') {
    poolSorted = poolSorted.filter((c) => rescanCustPhoneDigitsLen(c) >= 9);
  } else if (mode === 'without_phone') {
    poolSorted = poolSorted.filter((c) => rescanCustPhoneDigitsLen(c) < 9);
  }

  if (sort === 'newest_first') {
    contacts = poolSorted.slice(0, limit);
  } else {
    contacts = [...poolSorted].reverse().slice(0, limit);
  }
    ...
    try {
      ...
      if (!newPhone) {
        ...
        if (deleteLeadWhenNoPhone && c.lead_id) {
          const custIdBefore = rescanCustId(c) || c.customer_id || null;
        ...
        if (updErr) {
          ...
          custMapRescan[custId] = { ...(custMapRescan[custId] || { id: custId }), phone: newPhone };
        }
...
const FB_INBOX_CANNED_MAX = 80;

/** Tin soạn sẵn Hộp thư — lưu DB theo user */
r.get('/canned-replies', authMiddleware, async (req, res) => {
  try {
    const uid = req.user.userId || req.user.id;
    const { data, error } = await supabase
      .from('facebook_inbox_canned_replies')
      .select('id, title, body, sort_index, created_at, updated_at')
      .eq('user_id', uid)
      .order('sort_index', { ascending: true })
      .order('created_at', { ascending: false })
      .limit(FB_INBOX_CANNED_MAX);
    if (error) throw error;
    res.json({ items: data || [] });
  } catch (e) {
    res.status(500).json({ error: e.message });
...
r.post('/canned-replies', authMiddleware, async (req, res) => {
  try {
    const uid = req.user.userId || req.user.id;
    const title = String(req.body?.title || '').trim();
    const body = String(req.body?.body || req.body?.text || '').trim();
    if (!title || !body) return res.status(400).json({ error: 'Cần tiêu đề và nội dung' });
    const { count, error: cErr } = await supabase
      .from('facebook_inbox_canned_replies')
      .select('*', { count: 'exact', head: true })
      .eq('user_id', uid);
    if (cErr) throw cErr;
    if ((count || 0) >= FB_INBOX_CANNED_MAX) {
      return res.status(400).json({ error: `Tối đa ${FB_INBOX_CANNED_MAX} mẫu` });
    }
    const now = new Date().toISOString();
    const { data, error } = await supabase
      .from('facebook_inbox_canned_replies')
...
r.put('/canned-replies/:id', authMiddleware, async (req, res) => {
  try {
    const uid = req.user.userId || req.user.id;
    const title = String(req.body?.title || '').trim();
    const body = String(req.body?.body || req.body?.text || '').trim();
...
r.delete('/canned-replies/:id', authMiddleware, async (req, res) => {
  try {
    const uid = req.user.userId || req.user.id;
    const { data, error } = await supabase
      .from('facebook_inbox_canned_replies')
      ...
      .select('id');
    if (error) throw error;
    if (!data?.length) return res.status(404).json({ error: 'Không tìm thấy mẫu' });
    res.json({ ok: true });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
/** Nhập hàng loạt từ localStorage (tối đa còn slot) */
r.post('/canned-replies/import', authMiddleware, async (req, res) => {
  try {
    const uid = req.user.userId || req.user.id;
    const items = req.body?.items;
    if (!Array.isArray(items) || !items.length) {
      return res.status(400).json({ error: 'Cần mảng items { title, text hoặc body }' });
    }
    const { count: existing, error: cErr } = await supabase
      ...
      .eq('user_id', uid);
    if (cErr) throw cErr;
    let room = FB_INBOX_CANNED_MAX - (existing || 0);
    if (room <= 0) return res.json({ imported: 0, message: 'Đã đủ mẫu' });

    const now = new Date().toISOString();
    const rows = [];
    for (let i = 0; i < items.length && room > 0; i += 1) {
      const it = items[i];
      const title = String(it?.title || '').trim();
      const body = String(it?.body || it?.text || '').trim();
    ...
    const { data, error } = await supabase.from('facebook_inbox_canned_replies').insert(rows).select('id');
    if (error) throw error;
...
r.post('/tools/link-only-phones/preview', authMiddleware, async (req, res) => {
  try {
    ...
    if (!date_from || !date_to) {
      return res.status(400).json({ error: 'Cần date_from và date_to (ISO 8601)' });
    }
    const maxContactsToScan = parseMaxContactsLinkCleanup(max_contacts);
    const useStream = streamBody === true;
    ...
    if (useStream) {
      ...
      try {
        const out = await previewLinkOnlyPhones(supabase, {
          dateFrom: date_from,
          dateTo: date_to,
        ...
        send({ type: 'done', rows: out.rows, meta: out.meta, percent: 100 });
      } catch (e) {
...
r.post('/tools/link-only-phones/execute', authMiddleware, async (req, res) => {
  try {
    ...
    if (!date_from || !date_to) {
      return res.status(400).json({ error: 'Cần date_from và date_to giống lúc quét' });
    }
    if (!Array.isArray(items) || !items.length) {
      return res.status(400).json({ error: 'Cần mảng items (contact_id, phone)' });
    }
    const out = await executeLinkOnlyPhoneCleanup(supabase, items, {
