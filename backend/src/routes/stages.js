...
// GET /stages — all stages (include inactive for admin)
...
r.post('/', auth, async (req, res) => {
  try {
    ...
    if (oi === undefined || oi === null) {
      const { data: maxRow } = await supabase.from('workflow_stages').select('order_index').order('order_index', { ascending: false }).limit(1);
      oi = (maxRow?.[0]?.order_index || 0) + 1;
    }
...
// DELETE /stages/:id — only if no tasks/projects linked
...
r.post('/customer-statuses', auth, async (req, res) => {
  try {
    ...
    let oi = order_index;
    if (oi === undefined || oi === null) {
      const { data: maxRow } = await supabase.from('customer_statuses').select('order_index').order('order_index', { ascending: false }).limit(1);
      oi = (maxRow?.[0]?.order_index || 0) + 1;
    }
...
r.put('/customer-statuses/:id', auth, async (req, res) => {
  try {
    if (!['admin', 'manager'].includes(req.user.role))
      return res.status(403).json({ error: 'Không có quyền' });
...
r.delete('/customer-statuses/:id', auth, async (req, res) => {
  try {
    if (!['admin', 'manager'].includes(req.user.role))
      return res.status(403).json({ error: 'Không có quyền' });
    const { error } = await supabase.from('customer_statuses').delete().eq('id', req.params.id);
    if (error) throw error;
    res.json({ success: true });
  } catch (e) {
    res.status(500).json({ error: e.message });
...
r.put('/status-mapping', auth, async (req, res) => {
  try {
    if (!['admin', 'manager'].includes(req.user.role))
      return res.status(403).json({ error: 'Không có quyền' });
    const { mappings } = req.body; // [{ stage_id, customer_status_id }]
    ...
    for (const m of mappings) {
      ...
      // Delete existing mapping for this stage
