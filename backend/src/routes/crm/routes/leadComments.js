...
/** Deal gốc + deal con bàn giao VC/xưởng: một thread bình luận. */
async function resolveCommentThreadLeadIds(client, leadId) {
  const fallback = [leadId];
...
r.get('/leads/:id/comments', async (req, res) => {
  try {
    ...
    const threadLeadIds = await resolveCommentThreadLeadIds(supabase, leadId);
