...
function pingCostCogs(kind, id, userId) {
  if (!id) return;
  try {
    const { safeCost, syncCommercialDocCogs } = require('../../../helpers/costLedger');
    safeCost(() => syncCommercialDocCogs({ kind, id, actorUserId: userId }), `cogs:${kind}`);
  } catch (_) { /* ignore */ }
}
...
r.post('/quotations', async (req, res) => {
  try {
    ...
    pingCostCogs('quotation', quote.id, req.user.userId);
...
r.put('/quotations/:id', async (req, res) => {
  try {
    ...
    pingCostCogs('quotation', data.id, req.user.userId);
...
r.post('/quotations/:id/convert-to-order', async (req, res) => {
  try {
    ...
    pingCostCogs('order', order.id, req.user.userId);
    try {
      ...
      safeCost(async () => {
        const { data: entry } = await supabase
          .from('cost_entries')
          .select('id')
          .eq('source_table', 'quotations')
          .eq('source_row_id', req.params.id)
          .eq('source_key', 'crm.product_cogs')
          .maybeSingle();
        if (entry?.id) {
          await voidCostEntry(entry.id, { reason: 'BG đã chuyển ĐH', actorUserId: req.user.userId });
        }
      }, 'cogs:void-quote');
    } catch (_) { /* ignore */ }
...
r.put('/orders/:id', async (req, res) => {
  try {
    ...
    try { rcInvalidateTags(['crm:list']); } catch (_) { /* ignore */ }
    pingCostCogs('order', data.id, req.user.userId);
...
r.get('/quotations/:id/pdf', async (req, res) => {
  try {
    ...
    console.error('[quotations/:id/pdf]', e);
...
r.get('/invoices/:id/pdf', async (req, res) => {
  try {
    ...
    console.error('[invoices/:id/pdf]', e);
    if (!res.headersSent) res.status(500).json({ error: e.message });
    else res.destroy();
  }
});
