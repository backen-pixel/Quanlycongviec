...
r.put('/pipeline-stages-reorder', async (req, res) => {
  try {
    ...
    if (!items.length) return res.status(400).json({ error: 'Thiếu danh sách giai đoạn' });
    ...
    const normalized = items
      ...
      .filter((s) => s.id && Number.isFinite(s.order_index));
    if (!normalized.length) return res.status(400).json({ error: 'Danh sách giai đoạn không hợp lệ' });

    // 2 pha: tránh xung đột tạm thời khi đổi chỗ order_index tuần tự.
    for (const s of normalized) {
      const { error } = await supabase
        .from('crm_pipeline_stages')
        .update({ order_index: -(Math.abs(s.order_index) + 1000) })
        .eq('id', s.id);
      if (error) throw error;
    }
    for (const s of normalized) {
    ...
    // Trả stages đã cập nhật — FE áp dụng ngay, không phụ thuộc GET cache.
    ...
    const orderMap = new Map(normalized.map((s) => [s.id, s.order_index]));
    const stages = normalizePipelineStagesList(updatedRows || [])
      .map((row) => (
        orderMap.has(String(row.id))
          ? { ...row, order_index: orderMap.get(String(row.id)) }
          : row
      ))
      .sort((a, b) => (a.order_index ?? 0) - (b.order_index ?? 0));

    res.json({ message: 'Đã sắp xếp lại', stages });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
