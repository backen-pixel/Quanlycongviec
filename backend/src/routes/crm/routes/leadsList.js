...
r.post('/deadline-bucket-counts', async (req, res) => {
  try {
    ...
    // Endpoint này chỉ cần SỐ ĐẾM, nên dùng RPC đếm thẳng trong SQL. Snapshot đầy đủ
    // (`getOrBuildCrmDeadlineSnapshot`) phải nạp NGUYÊN dòng lead theo lô 200 + join
    // `customers` + quét `crm_tasks` (100.000+ dòng) chỉ để phân loại rồi bỏ đi phần thân —
    // đo trên dữ liệu thật: 2.127 dòng mất 3,0s; 6.237 dòng mất 7,1s (tuyến tính, tức
    // ~9s ở 8.000 dòng). Trong khi RPC `crm_deadline_bucket_counts` chạy 58ms trong SQL.
    //
    // Snapshot vẫn cần cho `/deadline-bucket-pages` (nó phải biết ID nào thuộc bucket nào
    // để cắt trang), nên chỉ dựng khi người dùng thật sự mở view Deadline.
    const fast = await fetchCrmDeadlineBucketCountsViaRpc(
      req, mergedQuery, type, openStageIds, config,
    );
    if (fast) {
      const fastCounts = Object.fromEntries(
        CRM_DEADLINE_BUCKET_KEYS.map((key) => [key, Number(fast.counts?.[key]) || 0]),
      );
      return res.json({
        type,
        counts: fastCounts,
        total: Object.values(fastCounts).reduce((s, n) => s + n, 0),
        complete: fast.complete,
      });
    }

    // RPC chưa cài (migration chưa chạy) → quay về snapshot như cũ.
...
r.post('/kpi-ledger-total', async (req, res) => {
  try {
    ...
    let result = null;
    if (!crmListUsesLegacyFilters(mergedQuery)) {
      // `idsOnly`: endpoint này chỉ lấy `row.id` ở ngay dưới. Không có cờ đó thì hàm còn
      // nạp nguyên dòng lead + join customers + cờ người dùng + deadline task đang mở
      // (quét bảng crm_tasks 100.000+ dòng) — toàn bộ đều bị vứt đi.
      result = await fetchCrmDeadlineCountRowsViaRpc(req, mergedQuery, type, MAX_ROWS, { idsOnly: true });
    }
