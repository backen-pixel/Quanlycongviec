...
r.get('/external-companies', requirePermission('projects', 'view'), async (req, res) => {
  try {
    ...
    const { ensureVptExternalCompanyCatalogForWorkshop } = require('../helpers/dealParticipantProduction');
    await ensureVptExternalCompanyCatalogForWorkshop(companyId, req.user?.userId);
...
/** Công ty CRM — chủ deal khi tạo đơn tại xưởng đối tác (HCB/Metalla). */
r.get('/client-companies', requirePermission('projects', 'view'), async (req, res) => {
  try {
    const companyId = effectiveWorkshopCompanyId(req, req.query.company_id || req.query.workshop_company_id);
    if (!companyId) return res.status(400).json({ error: 'Thiếu company_id' });
    const { listProductionClientCompanies } = require('../helpers/productionClientCompanies');
    const { ensureVptExternalCompanyCatalogForWorkshop } = require('../helpers/dealParticipantProduction');
    const { isMetallaOrHucabiCompanyIdSync } = require('../helpers/dealParticipantProduction');
    if (isMetallaOrHucabiCompanyIdSync(companyId)) {
      await ensureVptExternalCompanyCatalogForWorkshop(companyId, req.user?.userId);
    }
    const items = await listProductionClientCompanies(companyId);
    res.json({ items });
  } catch (e) {
    console.error('[production/client-companies GET]', e);
    res.status(500).json({ error: e.message });
  }
});
...
r.post('/workshop-intake', requirePermission('projects', 'create'), async (req, res) => {
  try {
    ...
    const result = await createWorkshopIntakeOrder({
      ...
      externalCatalogId: b.external_catalog_id || null,
    });
...
const CRM_DEALS_PROJECT_EMBED = 'crm_deals:crm_leads(id, type, title, region_id, created_at, assigned_to, lead_owner_id, external_company_name, external_company_id, sx_pipeline_stage_id, crm_region:company_regions(id, name, code), assignee:users!crm_leads_assigned_to_fkey(id, full_name, avatar), lead_owner:users!crm_leads_lead_owner_id_fkey(id, full_name, avatar))';
const CRM_DEALS_PROJECT_EMBED_LEGACY = 'crm_deals:crm_leads(id, type, title, region_id, created_at, assigned_to, lead_owner_id, sx_pipeline_stage_id, crm_region:company_regions(id, name, code), assignee:users!crm_leads_assigned_to_fkey(id, full_name, avatar), lead_owner:users!crm_leads_lead_owner_id_fkey(id, full_name, avatar))';
/** Mobile board: không embed assignee/avatar — payload nhỏ hơn nhiều khi 500–1000+ deal. */
