...
The current timestamp correction uses implementation generation **C3-R2**, not the
docs-only C4 acceptance step. Historical C3, C3-R1, and their existing ignored
...
Tracked files describe the evidence contract. Current generated artifacts belong in `runtime/candidates/c3-r2/`, which is Git-ignored so a local verification run does not dirty the frozen checkout.
...
PID/lock absent without manual cleanup. The static runner retains all ten C3-R1
checks and adds the timestamp verifier regression (eleven required groups).
The strict verifier requires these checks and lifecycle proof; a
