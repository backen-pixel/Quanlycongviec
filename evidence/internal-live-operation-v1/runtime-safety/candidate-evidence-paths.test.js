...
const prefix = 'evidence/internal-live-operation-v1/runtime-safety/runtime/candidates/c3-r1/';
const read = (file) => fs.readFileSync(path.join(repositoryDir, file), 'utf8');

test('corrected candidate producers and consumers cannot target historical C3 artifacts', () => {
  for (const file of ['candidate-binding.js', 'write-candidate-attestation.js', 'run-static-verification.js', 'verify-founder-local-runtime.js', 'verify-founder-local-lifecycle.js']) {
    ...
    assert.ok(source.includes("path.join(__dirname, 'runtime', 'candidates', 'c3-r1'"), file);
...
test('manifest and structural verifier use only the corrected fixed namespace', () => {
  ...
  assert.equal(manifest.generated_artifact_directory, 'runtime/candidates/c3-r1');
