...
export default function SystemBubbleSync() {
  ...
  const [prefs, setPrefs] = useState<VcMobilePrefs | null>(null);
  ...
  useEffect(() => {
    let cancelled = false;
    void loadVcMobilePrefs().then((p) => {
      if (!cancelled) setPrefs(p);
    });
    const sub = DeviceEventEmitter.addListener(VC_PREFS_CHANGED, (p: VcMobilePrefs) =>
      setPrefs(p),
    );
    return () => {
      cancelled = true;
