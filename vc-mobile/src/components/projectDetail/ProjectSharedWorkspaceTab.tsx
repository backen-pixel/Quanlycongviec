...
import {
  ...
  fetchSharedDealTasks,
  fetchSharedWorkspaceMembers,
  updateCrmAssignment,
  updateSharedDealTask,
  type CrmAssignment,
  type SharedCrmTask,
...
function memberModulesFromUser(user?: SharedWorkspaceMember['user'] | null): AssignModule[] {
  if (!user) return ['crm'];
...
function memberBelongsToModule(member: SharedWorkspaceMember, moduleId: AssignModule): boolean {
...
export default function ProjectSharedWorkspaceTab({ leadId, companyId }: Props) {
  const { colors: Colors } = useTheme();
  const styles = useMemo(() => makeStyles(Colors), [Colors]);

  const [moduleTab, setModuleTab] = useState<AssignModule>('logistics');
  const [members, setMembers] = useState<SharedWorkspaceMember[]>([]);
  const [assignments, setAssignments] = useState<CrmAssignment[]>([]);
  const [sharedTasks, setSharedTasks] = useState<SharedCrmTask[]>([]);
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try {
      const [mem, assigns, cols, tasks] = await Promise.all([
        fetchSharedWorkspaceMembers(leadId),
        fetchLeadAssignments(leadId),
        fetchAssignmentColumns(),
        fetchSharedDealTasks(leadId, 'logistics').catch(() => [] as SharedCrmTask[]),
      ]);
      setMembers(mem);
  ...
  const cycleTaskStatus = (t: SharedCrmTask) => {
    const order = ['pending', 'in_progress', 'completed', 'cancelled'];
    ...
    Alert.alert('Đổi trạng thái', `Chuyển «${t.title}» → ${STATUS_STAGE_LABEL[next] || next}?`, [
      { text: 'Huỷ', style: 'cancel' },
      {
        ...
        onPress: () => {
          void (async () => {
            try {
              await updateSharedDealTask(leadId, t.id, { status: next });
              await load(true);
            } catch (e) {
              Alert.alert('Lỗi', formatApiError(e));
            }
            ...
            <View key={a.id} style={styles.card}>
              ...
              <View style={styles.cardActions}>
                ...
                <Pressable onPress={() => removeAssignment(a)} hitSlop={6}>
                  <Text style={[styles.actionLink, { color: Colors.danger }]}>Xóa</Text>
                </Pressable>
              </View>
            </View>
          );
...
function makeStyles(C: AppColors) {
  return StyleSheet.create({
    ...
    modChipActive: { backgroundColor: C.primary, borderColor: C.primary },
    ...
    retryTxt: { color: C.primary, fontWeight: '700', marginTop: 6 },
    ...
    deadlineTxt: { fontSize: 11, color: C.warning, marginTop: 4, fontWeight: '600' },
    cardActions: { flexDirection: 'row', gap: 14, marginTop: 10 },
    actionLink: { fontSize: 12, fontWeight: '700', color: C.primary },
    ...
    choiceChipActive: { backgroundColor: C.primarySoft, borderColor: C.primary },
