...
/** G?i REST messenger t? overlay (không c?n m? React Native). */
object BubbleChatApi {
  ...
  fun fetchGroupMeta(ctx: Context, groupId: String): GroupMeta? {
    ...
    return try {
      ...
      GroupMeta(name, isDirect, if (isDirect) "Tr?c ti?p" else "Nhóm chat · realtime")
    } catch (_: Exception) {
      null
    }
  }
  ...
  private fun parseMessageRow(o: JSONObject, myUserId: String): ChatMessage {
    ...
    val sender = when {
      o.optBoolean("is_system", false) -> "H? th?ng"
      o.has("sender") -> o.optString("sender", "Ngu?i dùng")
      else -> o.optJSONObject("user")?.optString("full_name", "Ngu?i dùng") ?: "Ngu?i dùng"
    }
  ...
  private fun previewText(o: JSONObject): String {
    ...
    return when {
      ...
      type.contains("image", true) -> "?? Hình ?nh"
      type.contains("video", true) -> "?? Video"
      type.contains("file", true) || type.contains("document", true) -> "?? T?p dính kèm"
      type.contains("sticker", true) -> "Sticker"
      cleanApiString(o.optString("attachment_url", "")) != null -> "?? T?p dính kèm"
      else -> ""
    }
  }
