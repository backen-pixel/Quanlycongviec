...
/**
 * FCM data-only (bubble_wake) ? hi?n bong bóng ngay c? khi app chua m? JS.
 */
object BubbleFcmWake {
  ...
  fun handle(ctx: Context, data: Map<String, String>) {
    ...
    val title = data["title"]?.trim().orEmpty().ifBlank { "Tin nh?n" }
    val sender = data["sender_name"]?.trim().orEmpty().ifBlank { title }
    val message = data["message"]?.trim().orEmpty().ifBlank { "Có tin nh?n m?i" }
