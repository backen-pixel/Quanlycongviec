...
const {
  ...
  payloadSha256, createDraftAuthority, createFakeAuditWriter, createFakeDomainGuard, createFakeDraftAdapter, createDraftPreEffectHandoffProof,
} = require('./draft-pre-effect-handoff-proof');
...
function fixture(options = {}) {
  ...
  const content = {
    ...
    name: 'Synthetic controlled publish agent', created_by: 'builder.draft',
  ...
  registry.registerAgentPackage({ ...content, package_sha256: packageSha }, { actor_id: 'builder.draft', role: 'AUTHOR' });
  const transition = (status) => registry.transitionApproval(
    { agent_id: content.agent_id, version: content.version, to_status: status },
    { actor_id: 'review.draft', role: status === 'RETIRED' ? 'REGISTRY_ADMIN' : status === 'APPROVED' ? 'APPROVER' : 'REVIEWER' });
  ...
  const request = {
    ...
    action_id: 'ACT-DRAFT-001', tool_id: ACTION, policy_id: 'fake-policy', valid_until: FUTURE, effect_class: EFFECT_CLASS, agent_id: content.agent_id,
    ...
    payload, payload_sha256: payloadSha256(payload),
  };
  ...
  const data = {
    ...
    policy: { policy_id: 'fake-policy', policy_version: 'policy-1', company_id: request.company_id,
      ...
      role_permissions: { requester: [PERMISSION] } },
  };
  const authority = createDraftAuthority(data);
  ...
  const adapter = createFakeDraftAdapter({ mode: options.mode, afterAccept: () => options.afterAccept?.(f) });
  let reads = 0;
  ...
  const proof = createDraftPreEffectHandoffProof({ registry: gateRegistry, authority, audit, domain, adapter });
...
test('H01 ALLOW is an audited, zero-effect handoff; Application Service runs separately', () => {
  ...
  assert.equal(f.proof.listDrafts().length, 1);
...
test('D01 DRAFT requires scoped authority without creating or borrowing Publish approval', () => {
  const f = fixture();
  const c = f.allow();
...
for (const key of BINDINGS) test(`H05 permit binding rejects substituted ${key}`, () => {
  const f = fixture();
  const c = f.allow();
  assert.equal(c.decision, 'ALLOW');
...
test('H05 permit expiry cannot be extended through mutable authority fixtures', () => {
  ...
  f.change((d) => { d.now = '2026-09-02T03:00:02.000Z'; d.task.expires_at = FUTURE; });
...
test('D06 same action cannot create another draft with a substituted idempotency key', () => {
  const f = fixture();
  assert.equal(f.run().status, 'EXECUTED');
...
for (const [mode, status, state, count] of [
  ...
  assert.equal(f.proof.listDrafts().length, count);
...
for (const [label, make] of Object.entries(hostile)) for (const boundary of ['control', 'execution']) {
  test(`H08 ${label} at ${boundary} never escapes or controls audit metadata`, () => {
    ...
    zero(f, result);
    assert.match(result.correlation_id, /^draft-handoff-invalid-/);
...
test('H08 primary and secondary correlation, digests, immutable views and hash chains', () => {
  ...
  const view = f.proof.listDrafts(); view.length = 0;
  assert.equal(f.proof.listDrafts().length, 1);
...
test('H08 dependencies cannot be rebound to reset private idempotency state', () => {
  const f = fixture();
  assert.throws(() => createDraftPreEffectHandoffProof({ registry: f.registry, authority: f.authority,
...
test('D08 draft views remain readable beyond 200 distinct authorized actions', () => {
  const f = fixture();
  for (let i = 0; i < 201; i++) {
