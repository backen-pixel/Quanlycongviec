...
const ACTION = 'project.create_status_update_draft';
const PERMISSION = 'project.status_update.draft';
const EFFECT_CLASS = 'DRAFT';
...
function requestSnapshot(raw) {
  ...
  if (!r || Array.isArray(r) || typeof r !== 'object' ||
      ...
      REQUEST_KEYS.some((k) => !Object.hasOwn(r, k))) deny('INVALID_REQUEST');
  for (const k of REQUEST_KEYS.filter((key) => key !== 'payload')) {
...
function validateAgent(record, r) {
  ...
  if ([ACTION, PERMISSION, 'draft', EFFECT_CLASS].some((x) => includes(record.prohibited_actions, x))) deny('ACTION_PROHIBITED');
}
...
function validateContext(c, r) {
  ...
  if (!includes(c.policy.allowed_actions, ACTION) || !includes(c.policy.allowed_tools, ACTION) ||
      [ACTION, PERMISSION, EFFECT_CLASS, 'draft'].some((x) => includes(c.policy.prohibited_actions, x)) ||
...
function createDraftAuthority(initial) {
  const state = { data: freeze(copy(initial)), bound: false };
...
function createFakeDraftAdapter({ mode = 'SUCCESS', afterAccept = () => {} } = {}) {
  ...
  const adapter = Object.freeze({ listDrafts: () => state.effects.map((effect) => copy(effect)), callCount: () => state.calls });
  adapterStates.set(adapter, state);
  return adapter;
}

function createDraftPreEffectHandoffProof({ registry, authority, audit, domain, adapter }) {
  ...
  function safeEvent(r, event, status, reason, entry) {
    return {
      ...
      correlation_id: r ? r.correlation_id : `draft-handoff-invalid-${sequence}`,
  ...
  function respond(r, response, entry = null, permit = null, component = 'BOS_AI1') {
    const record = append(secondaryLedger, { ...safeEvent(r, 'RESULT', response.status, response.reason_code, entry), component });
    return freeze({ ...copy(response), ...(permit ? { permit } : {}),
      correlation_id: record.correlation_id, audit_id: `draft-handoff-secondary-${record.sequence}` });
  }

  function rejected(r, error, entry = null, component = 'BOS_AI1') {
    const known = decisions.get(error);
    const reason = known ? known.reason_code : 'INVALID_REQUEST';
    return respond(r, { decision: 'DENY', status: 'DENIED', reason_code: reason, effect_state: 'NONE' }, entry, null, component);
  }

  function inProgress(r, entry, component = 'BOS_AI1') {
    return respond(r, { status: 'IN_PROGRESS', reason_code: 'REQUEST_IN_PROGRESS', effect_state: 'NONE', duplicate: true }, entry, null, component);
  }
  ...
  function evaluate(raw) {
    ...
    try {
      ...
      const existingKey = actionKeys.get(r.action_id);
      ...
      if (!write(safeEvent(r, 'ACTION_INTENT', 'VALIDATED', 'INTENT_VALIDATED', entry)) ||
        ...
        if (reserved) { deliveries.delete(r.idempotency_key); actionKeys.delete(r.action_id); } else entry.state = 'PERMITTED';
      ...
      if (!entry.permit) {
        ...
        const body = { permit_id: `draft-handoff-permit-${++permitSequence}`, ...binding, issued_at: final.now,
      ...
      if (reserved) { deliveries.delete(r.idempotency_key); actionKeys.delete(r.action_id); }
  ...
  function finish(r, entry, status, reason) {
    ...
    return respond(r, entry.response, entry, null, 'APPLICATION_SERVICE');
  }
  ...
  function execute(permit, raw) {
    ...
    try {
      ...
      if (['ISSUING', 'EXECUTING'].includes(entry.state)) return inProgress(r, entry, 'APPLICATION_SERVICE');
      if (entry.state === 'COMPLETE') return respond(r, { ...entry.response, duplicate: true }, entry, null, 'APPLICATION_SERVICE');
      ...
      const effect = {
        effect_id: `fake-draft-effect-${fake.effects.length + 1}`,
      ...
      if (!executing) return rejected(r, error, entry, 'APPLICATION_SERVICE');
      const known = decisions.get(error);
  ...
  return Object.freeze({
    ...
    listDrafts: () => fake.effects.map((effect) => copy(effect)),
...
module.exports = {
  ...
  payloadSha256, createDraftAuthority, createFakeAuditWriter, createFakeDomainGuard,
  createFakeDraftAdapter, createDraftPreEffectHandoffProof,
};
