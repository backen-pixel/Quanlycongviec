 ...
 * File nguồn: crm-mobile-v2/google-services.json (tải từ Firebase Console).
 * Package phải là vn.tubeppro.crmobilev2.
 */
const fs = require('fs');
const path = require('path');
const { withDangerousMod, withAppBuildGradle } = require('@expo/config-plugins');

const PACKAGE = 'vn.tubeppro.crmobilev2';
const SOURCE_NAME = 'google-services.json';

function findSource(projectRoot) {
  const candidates = [
    path.join(projectRoot, SOURCE_NAME),
    path.join(projectRoot, 'google-services (crmv2).json'),
...
function validateGoogleServices(filePath) {
  try {
    ...
    if (!match) {
      return `Cần client package_name="${PACKAGE}" trong google-services.json (Firebase → Add Android app)`;
    }
    return null;
  } catch (e) {
  ...
  if (!src) {
    console.warn(
      `[withGoogleServices] Chưa có ${SOURCE_NAME}. ` +
        `Firebase Console → Add app Android → package ${PACKAGE}. ` +
        'Đặt file tại crm-mobile-v2/google-services.json rồi rebuild.',
    );
    return false;
  }
