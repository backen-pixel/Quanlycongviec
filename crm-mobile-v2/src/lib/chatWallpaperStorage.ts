...
const KEY = 'crmv2_chat_wallpapers_v1';
...
/** Cập nhật cache local từ URL server (socket / sau upload). */
export async function applyChatWallpaperUrl(threadId: string, url: string | null): Promise<ChatWallpaper> {
  if (!threadId) return { type: 'none' };
  const next = url ? resolveCachedUri(url) : null;
  await cacheLocal(threadId, next);
  return next ? { type: 'image', uri: next } : { type: 'none' };
}

function resolveCachedUri(url: string): string {
