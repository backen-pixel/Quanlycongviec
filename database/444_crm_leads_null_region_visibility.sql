-- 444: NV có crm_region_ids vẫn thấy deal/lead đã gán cho mình khi region_id IS NULL.
-- Trước đây RPC chỉ `l.region_id = ANY(p_region_ids)` → deal chưa gắn khu vực biến mất khỏi Kanban
-- dù assigned_to đúng (VD: LEAD-4299 Phúc Đạt / Nhiên).
-- Đồng thời backfill LEAD-4299 → Showroom TP.Hồ Chí Minh.
--
-- ⚠️ Supabase SQL Editor: chọn TOÀN BỘ file (Ctrl+A) rồi Run.

-- 1) Backfill deal thiếu khu vực (Phúc Đạt — Showroom HCM)
UPDATE crm_leads
SET region_id = '6e45a2aa-4664-4822-b5e9-1b2f85b424d1',
    updated_at = now()
WHERE id = '4c4a9e05-c394-4f27-b062-407d5efc46fc'
  AND region_id IS NULL
  AND company_id = '29677f68-967e-4256-92fd-492bb580e888';

-- 2) crm_leads_page_ids — giữ bản 430 (search customer.phone) + null-region cho assignee
  ...
  WITH base AS (
    ...
    WHERE l.type = p_type
      ...
      AND (
        ...
        OR (
          -- Deal/lead chưa gắn khu vực: vẫn hiện nếu đang lọc theo người phụ trách / thành viên
          l.region_id IS NULL
...
COMMENT ON FUNCTION public.crm_leads_page_ids(
  ...
  'Phân trang GET /api/crm/leads. p_region_ids: lead thuộc khu vực HOẶC (region_id NULL + khớp p_assigned_to).';
...
-- 3) crm_leads_stage_counts — cùng logic null-region
...
COMMENT ON FUNCTION public.crm_leads_stage_counts(
  ...
  'Đếm lead/deal theo stage — null region vẫn đếm khi khớp p_assigned_to.';
