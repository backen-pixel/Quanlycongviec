-- 430: Fix tìm kiếm lead/deal theo SĐT khi số điện thoại chỉ lưu ở customers.phone
-- (crm_leads.phone hầu như luôn NULL — số điện thoại thật nằm ở bảng customers qua customer_id).
-- Trước bản này, p_search trong crm_leads_page_ids / crm_leads_stage_counts chỉ so khớp
-- l.phone (cột trên crm_leads) nên tìm theo SĐT của lead (VD: lead Facebook Auto Tool) luôn ra rỗng.
-- Bổ sung so khớp thêm c.phone (số điện thoại khách hàng liên kết).
--
-- ⚠️ Supabase SQL Editor: chọn TOÀN BỘ file (Ctrl+A) rồi Run.

-- crm_leads_page_ids (bản 364 + fix search theo customer.phone)
...
-- crm_leads_stage_counts (bản 365 + fix search theo customer.phone)
...
COMMENT ON FUNCTION public.crm_leads_stage_counts(
  ...
  'Đếm lead/deal + tổng GT theo stage_id — Kanban + báo cáo pipeline mở. Search khớp cả SĐT/tên khách hàng liên kết.';
