-- 470: Full-parity CRM Kanban filters on RPC
-- - Mở rộng p_search (khớp UI web: mô tả, địa chỉ, email, công ty KH, NV, nguồn)
-- - Thêm p_assignee_name (ILIKE assignee / lead_owner)
-- - Thêm p_region_unassigned (lọc region_id IS NULL — «Chưa gán khu vực»)
--
-- ⚠️ Supabase SQL Editor: chọn TOÀN BỘ file (Ctrl+A) rồi Run.
-- PostgREST: DROP overload cũ rồi CREATE mới để tránh trùng signature.
...
COMMENT ON FUNCTION public.crm_leads_page_ids(
  ...
  'Phân trang GET /api/crm/leads. p_assignee_name ILIKE NV; p_region_unassigned = Chưa gán khu vực.';
...
COMMENT ON FUNCTION public.crm_leads_stage_counts(
  ...
  'Đếm lead/deal theo cột. Hỗ trợ p_assignee_name + p_region_unassigned.';
