-- 557: Mở rộng crm_filter_summary để cộng thêm SUM(estimated_value) và SUM(weighted_value)
-- theo từng stage_id (bên cạnh COUNT đã có). Dùng để tính chính xác các ô KPI Deal trên
-- CRM Dashboard (Tổng Deal, Đang xử lý, Hủy/Thua, Giá trị dự kiến, Giá trị kỳ vọng) trên
-- TOÀN BỘ deal khớp bộ lọc, thay vì chỉ ~40 thẻ đã tải vào Kanban do cuộn lười.
...
COMMENT ON FUNCTION public.crm_filter_summary(
  uuid, uuid, uuid, boolean, boolean, uuid, text, text, text, text,
  uuid[], boolean, uuid[], uuid[], text, uuid, text, text
) IS 'Một lần quét cho tổng Lead/Deal, bucket SĐT, stage counts và (deal) tổng giá trị/giá trị kỳ vọng theo bộ lọc CRM Dashboard.';
