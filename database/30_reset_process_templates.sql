...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO company_process_checklists (task_id, title, order_index) VALUES
      (task_id, 'Ghi nhận tên, SĐT, email KH', 1),
      (task_id, 'Xác nhận nhu cầu (loại tủ, phong cách)', 2),
      (task_id, 'Ghi nhận ngân sách dự kiến', 3);

    -- Nhiệm vụ 1.2
    INSERT INTO company_process_tasks (id, process_id, title, priority, order_index)
    VALUES (gen_random_uuid(), proc_id, 'Tư vấn sản phẩm & vật liệu', 'high', 2) RETURNING id INTO task_id;
    INSERT INTO company_process_checklists (task_id, title, order_index) VALUES
      (task_id, 'Giới thiệu các dòng sản phẩm phù hợp', 1),
      (task_id, 'Tư vấn vật liệu (gỗ, phụ kiện, mặt đá)', 2),
      (task_id, 'Gửi catalog / hình ảnh mẫu', 3);

    -- Nhiệm vụ 1.3
    INSERT INTO company_process_tasks (id, process_id, title, priority, order_index)
    VALUES (gen_random_uuid(), proc_id, 'Khảo sát thực tế', 'medium', 3) RETURNING id INTO task_id;
    INSERT INTO company_process_checklists (task_id, title, order_index) VALUES
      (task_id, 'Hẹn lịch khảo sát với KH', 1),
      (task_id, 'Đo kích thước thực tế', 2),
      (task_id, 'Chụp ảnh hiện trạng', 3),
      (task_id, 'Ghi nhận yêu cầu đặc biệt', 4);

    -- Nhiệm vụ 1.4
    INSERT INTO company_process_tasks (id, process_id, title, priority, order_index)
    VALUES (gen_random_uuid(), proc_id, 'Ghi nhận nhu cầu chi tiết', 'medium', 4) RETURNING id INTO task_id;
    INSERT INTO company_process_checklists (task_id, title, order_index) VALUES
      (task_id, 'Tổng hợp yêu cầu KH vào hồ sơ', 1),
      (task_id, 'Xác nhận lại với KH qua điện thoại/Zalo', 2);
    ...
    INSERT INTO company_process_checklists (task_id, title, order_index) VALUES
      (task_id, 'Vẽ layout 2D từ số đo', 1),
      (task_id, 'Chọn phong cách & màu sắc', 2);

    INSERT INTO company_process_tasks (id, process_id, title, priority, order_index)
    VALUES (gen_random_uuid(), proc_id, 'Thiết kế 3D render', 'high', 2) RETURNING id INTO task_id;
    INSERT INTO company_process_checklists (task_id, title, order_index) VALUES
      (task_id, 'Dựng 3D từ bản vẽ 2D', 1),
      (task_id, 'Render hình ảnh chất lượng cao', 2),
      (task_id, 'Gửi cho KH xem trước', 3);

    INSERT INTO company_process_tasks (id, process_id, title, priority, order_index)
    VALUES (gen_random_uuid(), proc_id, 'Chỉnh sửa theo feedback KH', 'medium', 3) RETURNING id INTO task_id;
    INSERT INTO company_process_checklists (task_id, title, order_index) VALUES
      (task_id, 'Ghi nhận feedback từ KH', 1),
      (task_id, 'Chỉnh sửa bản vẽ', 2),
      (task_id, 'KH duyệt bản vẽ cuối', 3);

    INSERT INTO company_process_tasks (id, process_id, title, priority, order_index)
    VALUES (gen_random_uuid(), proc_id, 'Xuất bản vẽ kỹ thuật & CNC', 'high', 4) RETURNING id INTO task_id;
    INSERT INTO company_process_checklists (task_id, title, order_index) VALUES
      (task_id, 'Hoàn thiện bản vẽ kỹ thuật chi tiết', 1),
      (task_id, 'Xuất file CNC cho sản xuất', 2);
    ...
    INSERT INTO company_process_checklists (task_id, title, order_index) VALUES
      (task_id, 'Bóc tách vật tư từ bản vẽ kỹ thuật', 1),
      (task_id, 'Tính toán chi phí vật liệu + gia công', 2),
      (task_id, 'Lập báo giá chi tiết', 3),
      (task_id, 'Gửi báo giá cho KH', 4);

    INSERT INTO company_process_tasks (id, process_id, title, priority, order_index)
    VALUES (gen_random_uuid(), proc_id, 'Thương lượng & chốt giá', 'medium', 2) RETURNING id INTO task_id;
    INSERT INTO company_process_checklists (task_id, title, order_index) VALUES
      (task_id, 'Thương lượng giá với KH', 1),
      (task_id, 'Chốt giá cuối cùng', 2);

    INSERT INTO company_process_tasks (id, process_id, title, priority, order_index)
    VALUES (gen_random_uuid(), proc_id, 'Ký hợp đồng & thu cọc', 'urgent', 3) RETURNING id INTO task_id;
    INSERT INTO company_process_checklists (task_id, title, order_index) VALUES
      (task_id, 'Soạn hợp đồng', 1),
      (task_id, 'KH review & ký hợp đồng', 2),
      (task_id, 'Thu tiền đặt cọc', 3),
      (task_id, 'Scan / lưu HĐ đã ký', 4);
...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO company_process_checklists (task_id, title, order_index) VALUES
      (task_id, 'Đặt mua NVL theo bảng bóc tách', 1),
      (task_id, 'Kiểm tra NVL nhập kho', 2),
      (task_id, 'Phân loại & sắp xếp NVL', 3);

    INSERT INTO company_process_tasks (id, process_id, title, priority, order_index)
    VALUES (gen_random_uuid(), proc_id, 'Gia công CNC & dán cạnh', 'high', 2) RETURNING id INTO task_id;
    INSERT INTO company_process_checklists (task_id, title, order_index) VALUES
      (task_id, 'Cắt CNC theo bản vẽ kỹ thuật', 1),
      (task_id, 'Dán cạnh / phủ bề mặt', 2),
      (task_id, 'Kiểm tra kích thước sau cắt', 3);

    INSERT INTO company_process_tasks (id, process_id, title, priority, order_index)
    VALUES (gen_random_uuid(), proc_id, 'Lắp ráp & hoàn thiện', 'high', 3) RETURNING id INTO task_id;
    INSERT INTO company_process_checklists (task_id, title, order_index) VALUES
      (task_id, 'Lắp ráp khung tủ', 1),
      (task_id, 'Lắp phụ kiện (bản lề, ray, giảm chấn)', 2),
      (task_id, 'Sơn / hoàn thiện bề mặt (nếu có)', 3);

    INSERT INTO company_process_tasks (id, process_id, title, priority, order_index)
    VALUES (gen_random_uuid(), proc_id, 'Kiểm tra QC & đóng gói', 'urgent', 4) RETURNING id INTO task_id;
    INSERT INTO company_process_checklists (task_id, title, order_index) VALUES
      (task_id, 'Kiểm tra chất lượng tổng thể', 1),
      (task_id, 'Đối chiếu với bản vẽ kỹ thuật', 2),
      (task_id, 'Đóng gói & dán nhãn', 3),
      (task_id, 'Chuyển kho chờ xuất', 4);
...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO company_process_checklists (task_id, title, order_index) VALUES
      (task_id, 'Kiểm tra hàng trước khi xuất kho', 1),
      (task_id, 'Đối chiếu với danh sách đóng gói', 2),
      (task_id, 'Sắp xếp xe vận chuyển', 3),
      (task_id, 'Giao hàng đến công trình', 4);

    INSERT INTO company_process_tasks (id, process_id, title, priority, order_index)
    VALUES (gen_random_uuid(), proc_id, 'Lắp đặt tại công trình', 'high', 2) RETURNING id INTO task_id;
    INSERT INTO company_process_checklists (task_id, title, order_index) VALUES
      (task_id, 'Kiểm tra hiện trạng công trình', 1),
      (task_id, 'Lắp đặt tủ bếp theo bản vẽ', 2),
      (task_id, 'Lắp đặt mặt đá / thiết bị', 3),
      (task_id, 'Căn chỉnh & hoàn thiện chi tiết', 4);

    INSERT INTO company_process_tasks (id, process_id, title, priority, order_index)
    VALUES (gen_random_uuid(), proc_id, 'Nghiệm thu & bàn giao', 'urgent', 3) RETURNING id INTO task_id;
    INSERT INTO company_process_checklists (task_id, title, order_index) VALUES
      (task_id, 'Nghiệm thu với khách hàng', 1),
      (task_id, 'Chụp ảnh hoàn thiện công trình', 2),
      (task_id, 'KH ký biên bản nghiệm thu', 3),
      (task_id, 'Dọn dẹp vệ sinh công trình', 4);
    ...
    INSERT INTO company_process_checklists (task_id, title, order_index) VALUES
      (task_id, 'Gọi điện hỏi thăm sau 3 ngày', 1),
      (task_id, 'Gọi điện hỏi thăm sau 1 tháng', 2),
      (task_id, 'Ghi nhận phản hồi / khiếu nại', 3);

    INSERT INTO company_process_tasks (id, process_id, title, priority, order_index)
    VALUES (gen_random_uuid(), proc_id, 'Xử lý bảo hành', 'high', 2) RETURNING id INTO task_id;
    INSERT INTO company_process_checklists (task_id, title, order_index) VALUES
      (task_id, 'Tiếp nhận yêu cầu bảo hành', 1),
      (task_id, 'Đánh giá & lên phương án xử lý', 2),
      (task_id, 'Thực hiện bảo hành', 3),
      (task_id, 'KH xác nhận hoàn thành', 4);

    INSERT INTO company_process_tasks (id, process_id, title, priority, order_index)
    VALUES (gen_random_uuid(), proc_id, 'Xin đánh giá & giới thiệu', 'low', 3) RETURNING id INTO task_id;
    INSERT INTO company_process_checklists (task_id, title, order_index) VALUES
      (task_id, 'Xin review / đánh giá từ KH', 1),
      (task_id, 'Đề xuất cross-sell / upsell', 2),
      (task_id, 'Xin giới thiệu KH mới', 3);
