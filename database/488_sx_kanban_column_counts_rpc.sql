-- 488: Gom counts Kanban SX theo sx_kanban_column_id trong một RPC (GROUP BY).
-- Thay N lần head-count từ Render → Supabase (RTT cao làm board giật).
...
COMMENT ON FUNCTION public.sx_kanban_column_counts IS
  'SX Kanban: aggregate project counts by sx_kanban_column_id in one roundtrip.';
