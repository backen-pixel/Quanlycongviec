-- Preview tin cuối gồm log cuộc gọi (is_system / message_type call)
    ...
    (
      SELECT CASE
        WHEN COALESCE(mm.recalled_at, NULL) IS NOT NULL OR COALESCE(mm.is_recalled, false) THEN
          ...
          END
        WHEN BTRIM(COALESCE(mm.content, '')) LIKE ':call_log:%' THEN '📞 Cuộc gọi'
        WHEN COALESCE(mm.message_type, '') = 'call' THEN '📞 Cuộc gọi'
      ...
      WHERE mm.group_id = m.group_id
        AND (
          COALESCE(mm.is_system, false) = false
          OR COALESCE(mm.message_type, '') = 'call'
          OR BTRIM(COALESCE(mm.content, '')) LIKE ':call_log:%'
        )
