-- 488: Deadline lead/deal chỉ lấy nhiệm vụ thuộc cột hiện tại.
-- Nhiệm vụ không gắn pipeline_stage_id vẫn được xem là nhiệm vụ chung của lead.
...
BEGIN
  ...
  task_deadlines AS (
    ...
    JOIN selected s ON s.id = t.lead_id
    WHERE t.status IN ('pending', 'in_progress')
      AND t.deadline IS NOT NULL
      AND (t.pipeline_stage_id IS NULL OR t.pipeline_stage_id = s.stage_id)
    GROUP BY t.lead_id
  ),
  effective AS (
    SELECT
...
) IS 'Đếm bucket Deadline CRM, chỉ dùng nhiệm vụ thuộc cột hiện tại hoặc không gắn cột';

COMMENT ON FUNCTION public.crm_deadline_bucket_page_ids(
  uuid[], uuid[], uuid, jsonb, boolean, integer, integer, integer, integer
) IS 'Phân trang Deadline CRM, chỉ dùng nhiệm vụ thuộc cột hiện tại hoặc không gắn cột';
