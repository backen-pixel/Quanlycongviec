...
-- ─── 5. Trigger 2a: Đóng segment stage — ghi stage_changed (SLA theo stage đang ở)
--    Bản ghi history khi đóng là segment "thời gian ở stage X": pair (from_stage → to_stage)
--    trong đó to_stage_id = stage đang đo → SLA lấy theo to_stage_id (KHÔNG phải from_stage_id).
--    Won/Lost không xử lý ở đây — xem Trigger 2b (INSERT dòng chuyển tiếp vào đích).
...
DECLARE
  ...
  v_calc RECORD;
BEGIN
  -- AFTER UPDATE khi exited_at được set (đóng segment đang mở)
  ...
  -- SLA của stage đang ở trong segment (= to_stage_id của dòng đang đóng)
...
-- ─── 5b. Won/Lost — INSERT dòng transition có to_canonical_slug = đích (contract_signed / lost)
