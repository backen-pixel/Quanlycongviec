-- 367: CRM RPC � l?c theo company_id ho?c external_company_id
-- Ch?y sau 364 v� 366

-- 364: Kanban CRM — NV thấy lead/deal nếu là thành viên nhóm (lead_members), không chỉ phụ trách.
-- Khớp enforceCrmDealAssigneeAccess (tab chi tiết) với danh sách Pipeline.

-- crm_leads_page_ids (bản 325 + ẩn deal con)
...
BEGIN
  ...
  WITH base AS (
    ...
    WHERE l.type = p_type
      ...
      AND (p_company_id IS NULL OR l.company_id = p_company_id OR l.external_company_id = p_company_id)
...
-- crm_leads_stage_counts (bản 326)
