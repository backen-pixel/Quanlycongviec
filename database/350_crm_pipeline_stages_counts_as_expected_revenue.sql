...
-- Cờ riêng cho "Giá trị kỳ vọng" trên CRM dashboard (deal pipeline).
--
-- Quy ước đọc dữ liệu:
--   - NẾU pipeline có ÍT NHẤT một stage `counts_as_expected_revenue = true`
--     → ô "Giá trị kỳ vọng" chỉ cộng các stage được tick.
...
COMMENT ON COLUMN crm_pipeline_stages.counts_as_expected_revenue IS
  'Khi TRUE: cộng vào ô "Giá trị kỳ vọng" trên CRM dashboard. NULL/FALSE = không cộng. Nếu không stage nào set, dashboard fallback loại Thua/Thắng/Hoàn thành DT.';
