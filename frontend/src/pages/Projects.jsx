              ...
              {STATUS_COLUMNS.map(col => (
                <div key={col.id} className="flex flex-col">
                  <div className="rounded-t-xl p-3 border border-b-0 bg-white" style={{ borderTopColor: col.color, borderTopWidth: '3px' }}>
                    ...
                    <span className="text-xs text-gray-400">{projectsByStatus[col.id].length} dự án</span>
                  </div>
                  <div className="flex-1 rounded-b-xl border p-2 space-y-2 bg-gray-50/50 overflow-y-auto" style={{ minHeight: '400px', maxHeight: '70vh' }}>
                    {projectsByStatus[col.id].map(proj => (
                      <Link to={`/projects/${proj.id}`} key={proj.id} className="block bg-white rounded-lg border p-3 hover:shadow-md transition-all">
                        <span className="text-xs font-bold text-blue-600">{proj.code}</span>
                        <h3 className="text-sm font-semibold text-gray-900 my-1">{proj.name}</h3>
                        {proj.customers?.full_name && <p className="text-xs text-gray-500">👤 {proj.customers.full_name}</p>}
                        {proj.company && <p className="text-[10px] text-indigo-600">🏢 {proj.company.short_name || proj.company.name}</p>}
                        {proj.current_stage && <span className="inline-block mt-2 text-[10px] px-1.5 py-0.5 rounded bg-gray-100">{proj.current_stage.name}</span>}
                      </Link>
                    ))}
