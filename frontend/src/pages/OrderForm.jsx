...
export default function OrderForm() {
  ...
  const addProductToItems = (p) => {
    setItems(prev => [...prev, {
      ...
      promo_code: '', is_promo: false, spec_factor: 0,
    }]);
  };
  ...
  const calcs = useMemo(() => {
    const rows = items.map(i => {
      if (i.row_type === 'section') return { ...i, amount: 0, gross_amount: 0, discount_amount: 0, vat_amount: 0, tax_amount: 0, total: 0, notes: '__SECTION__' };
              ...
              {(() => { let itemNo = 0; return items.map((item, idx) => {
                if (item.row_type === 'section') return (
                  <tr key={idx} className="bg-indigo-50 border-b border-indigo-200">
                    <td colSpan={16} className="py-1.5 px-2">
                      <input
                        value={item.name}
