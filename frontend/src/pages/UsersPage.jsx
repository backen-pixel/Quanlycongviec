...
import { Plus, Search, Mail, Phone, MapPin, Calendar, Trash2, Edit, Users as UsersIcon, Briefcase, Shield } from 'lucide-react';
import { formatDate, getInitials, avatarColor, formatVND } from '../lib/utils';
...
export default function UsersPage() {
  ...
  const load = () => {
    setLoading(true);
    api.get('/users', { params: { search: search || undefined, role: filterRole || undefined, department_id: filterDept || undefined } })
      .then(r => { setUsers(r.data.users || []); setStats(r.data.stats || {}); })
      .catch(() => {}).finally(() => setLoading(false));
  };
  useEffect(() => { load(); api.get('/users/departments').then(r => setDepartments(r.data.departments || [])); }, []);
  useEffect(load, [filterRole, filterDept]);

  const deactivate = async (e, id, name) => {
    e.stopPropagation();
    ...
    <div className="space-y-5 max-w-7xl">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">{stats.total || 0} nhân viên</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16"><UsersIcon className="h-12 w-12 mx-auto text-gray-300 mb-3" /><p className="text-sm text-gray-400">Chưa có nhân viên</p></div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {users.map((u, i) => (
            <div key={u.id} onClick={() => setShowDetail(u.id)}
              className="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-4 hover:shadow-md hover:border-gray-300 transition-all cursor-pointer animate-fade-in group"
              ...
              <div className="flex items-center gap-1 shrink-0">
                <button onClick={(e) => { e.stopPropagation(); setEditUser(u); setShowCreate(true); }}
                  className="opacity-0 group-hover:opacity-100 w-7 h-7 rounded-lg hover:bg-gray-100 flex items-center justify-center text-gray-400 hover:text-blue-500 cursor-pointer">
                  <Edit className="h-3.5 w-3.5" /></button>
                <button onClick={(e) => deactivate(e, u.id, u.full_name)}
    ...
    <Modal open={open} onClose={onClose} title={editUser ? 'Sửa nhân viên' : 'Thêm nhân viên mới'} size="md">
      <form onSubmit={submit} className="space-y-4 max-h-[70vh] overflow-y-auto pr-1">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div><label className="block text-sm font-medium mb-1">Mật khẩu {editUser ? '(để trống = giữ nguyên)' : ''}</label><input type="password" value={form.password || ''} onChange={e => set('password', e.target.value)} className="input" placeholder={editUser ? '••••••' : 'tubep123'} /></div>
          ...
          <div><label className="block text-sm font-medium mb-1">Phòng ban</label>
            <select value={form.department_id || ''} onChange={e => set('department_id', e.target.value || null)} className="input">
              <option value="">— Chọn —</option>{departments?.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}
            </select></div>
...
function StaffDetailModal({ userId, open, onClose }) {
  ...
  const [loading, setLoading] = useState(true);
  useEffect(() => { if (!open || !userId) return; setLoading(true); api.get(`/users/${userId}`).then(r => setUser(r.data.user)).finally(() => setLoading(false)); }, [open, userId]);
  if (!open) return null;
  return (
    <Modal open={open} onClose={onClose} title={user?.full_name || 'Nhân viên'} size="md">
      {loading || !user ? <div className="flex items-center justify-center h-32"><svg className="animate-spin h-6 w-6 text-gray-400" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4a4 4 0 00-4 4H4z"/></svg></div> : (
        <div className="space-y-5">
          ...
          {user.address && <p className="text-xs text-gray-500"><MapPin className="inline h-3 w-3 mr-1" />{user.address}</p>}
        </div>
      )}
    </Modal>
  );
