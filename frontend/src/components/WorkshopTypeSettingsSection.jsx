 ...
 * @param {{ moduleContext: 'production' | 'logistics', accent: 'teal' | 'orange', companyId?: string, onCompanyIdChange?: (id: string) => void, onTypesChanged?: () => void, layout?: 'default' | 'sidebar' }} props
 * Khi truyền `onCompanyIdChange`, công ty do trang cha điều khiển (ẩn dropdown trong section).
 */
export default function WorkshopTypeSettingsSection({ moduleContext, accent = 'teal', companyId: companyIdProp, onCompanyIdChange, onTypesChanged, layout = 'default' }) {
  const { user } = useAuth();
  ...
  const iconBg = accent === 'orange' ? 'bg-orange-100 text-orange-700' : 'bg-teal-100 text-teal-700';
  ...
  return (
    <div className={`bg-white rounded-xl border overflow-hidden ${border} ${sidebar ? 'shadow-sm' : ''}`}>
      <div className={`flex flex-wrap items-center justify-between gap-2 p-4 border-b bg-gradient-to-r ${bar} to-white`}>
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div>
            ...
            <p className={`text-[10px] text-gray-500 ${sidebar ? 'leading-snug' : ''}`}>
              Gắn cho từng dự án; lọc trên dashboard. Theo công ty (giống phân loại Lead/Deal CRM).
            </p>
          </div>
        </div>
      ...
      {loading ? (
        ...
        <div className={`p-4 space-y-3 ${sidebar ? 'max-h-[min(70vh,720px)] overflow-y-auto' : ''}`}>
          <div className={`grid gap-2 items-end border border-dashed border-gray-200 rounded-lg p-3 bg-gray-50/80 ${sidebar ? 'grid-cols-1' : 'sm:grid-cols-12'}`}>
            <label className={`${sidebar ? '' : 'sm:col-span-5'} text-[11px] text-gray-700`}>
            ...
            <label className={`${sidebar ? '' : 'sm:col-span-4'} text-[11px] text-gray-700`}>
            ...
            <div className={sidebar ? '' : 'sm:col-span-3'}>
              <button
                type="button"
                onClick={addType}
          ...
          {types.length === 0 ? (
            ...
            <ul className="space-y-2">
              {types.map((t) => (
                <li
                  key={t.id}
                  className={`rounded-lg border border-gray-100 hover:border-gray-200 ${!t.is_active ? 'opacity-50' : ''} ${sidebar ? 'p-2.5 space-y-2 bg-white' : 'flex flex-wrap items-center gap-2 p-2'}`}
                >
                  {sidebar ? (
                    <>
                      <input
                      ...
                      <div className="flex flex-wrap items-center gap-1.5">
                        <select
                          ...
                          className="flex-1 min-w-[7rem] border border-gray-200 rounded-lg px-2 py-1.5 text-xs bg-white"
                        >
                          {appliesOptions.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
                        ...
                        <button
                          ...
                          className={`h-7 w-7 rounded-md flex items-center justify-center border ${border} ${accent === 'orange' ? 'text-orange-800' : 'text-teal-800'} hover:bg-gray-50 cursor-pointer disabled:opacity-50`}
                          title="Lưu"
                        >
                        ...
                        <button
                          type="button"
                          onClick={() => delType(t)}
                          className="h-7 w-7 rounded-md flex items-center justify-center text-red-600 hover:bg-red-50 cursor-pointer"
                          title="Xóa"
                        >
                      ...
                      <input
                        ...
                        placeholder="Mô tả / gợi ý (CRM → Sản xuất)…"
                        className="w-full border border-gray-200 rounded-lg px-2 py-1.5 text-[11px] text-gray-600"
                      />
                    </>
                  ) : (
                    <>
