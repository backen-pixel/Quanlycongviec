    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div>
            <h2 className="text-lg font-bold text-gray-900">🏭 Tạo dự án sản xuất mới</h2>
            <p className="text-xs text-gray-500 mt-0.5">Điền thông tin để tạo dự án vào xưởng</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="VD: Tủ bếp anh Minh - 123 Lê Văn Việt"
          ...
          <div>
            ...
            {selectedCustomer ? (
              <div className="flex items-center gap-2 px-3 py-2 bg-blue-50 border border-blue-200 rounded-lg">
                <span className="text-sm font-medium text-blue-800 flex-1">{selectedCustomer.full_name}</span>
                {selectedCustomer.phone && <span className="text-xs text-blue-600">📞 {selectedCustomer.phone}</span>}
                <button type="button" onClick={() => { setSelectedCustomer(null); setCustomerSearch(''); }} className="text-blue-400 hover:text-blue-600 cursor-pointer">
                  ...
                  <div className="absolute top-full mt-1 left-0 right-0 bg-white border border-gray-200 rounded-lg shadow-lg z-10 overflow-hidden">
                    {customerResults.map(c => (
                      <button
                        ...
                        className="w-full flex items-center gap-3 px-3 py-2.5 hover:bg-blue-50 transition text-left border-b border-gray-50 last:border-0"
                      >
          ...
          {/* Phụ trách SX & Deadline */}
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Phụ trách sản xuất</label>
              <select
                value={formData.production_person_id}
                onChange={e => set('production_person_id', e.target.value)}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
              >
          ...
          <div className="flex items-center gap-3 pt-2 border-t border-gray-100">
            <button
              type="submit"
              disabled={saving}
              className="flex-1 px-4 py-2.5 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 transition-all duration-200 disabled:opacity-50 text-sm cursor-pointer"
            >
              {saving ? 'Đang tạo...' : '🏭 Tạo dự án SX'}
            </button>
            <button
              type="button"
              onClick={onClose}
