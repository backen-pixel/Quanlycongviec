...
import {
  Clock, CheckSquare, MessageSquare, Users, Send, Trash2,
  User, Eye, Plus, Calendar, AlertTriangle, Paperclip
} from 'lucide-react';
        ...
        <div className="space-y-5">
          ...
          <div>
            <div className="flex items-center gap-2 mb-2 flex-wrap">
              ...
              {task.attachments?.length > 0 && <span className="text-xs text-gray-400 flex items-center gap-1"><Paperclip className="h-3 w-3" /> {task.attachments.length} file</span>}
            </div>
          ...
          {task.description && <div className="bg-gray-50 rounded-lg p-4 text-sm text-gray-700">{task.description}</div>}
          ...
          <div className="flex gap-1 border-b border-gray-200">
            {tabs.map(t => (
              <button key={t.id} onClick={() => setActiveTab(t.id)}
                className={`flex items-center gap-1.5 px-3 py-2 text-sm font-medium border-b-2 cursor-pointer ${
          ...
          {activeTab === 'people' && (
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <RoleCard icon={User} label="Người giao việc" user={task.creator} />
                <RoleCard icon={User} label="Người thực hiện" user={task.assignee} />
              </div>
              <div>
                <h4 className="text-sm font-medium text-gray-700 mb-2 flex items-center gap-1"><Users className="h-4 w-4" /> Người hỗ trợ</h4>
              ...
              <div>
                <h4 className="text-sm font-medium text-gray-700 mb-2 flex items-center gap-1"><Eye className="h-4 w-4" /> Người quan sát</h4>
                <div className="space-y-1">
  ...
  return (
    <div className="bg-gray-50 rounded-lg p-3">
      ...
      <div className={`flex items-center gap-1.5 ${danger ? 'text-red-600' : 'text-gray-900'}`}>
        ...
        {badge ? <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${badge}`}>{value || '—'}</span>
...
function RoleCard({ icon: Icon, label, user }) {
  return (
    <div className="flex items-center gap-3 bg-gray-50 rounded-lg p-3">
      ...
      <div><p className="text-[11px] text-gray-500">{label}</p><p className="text-sm font-medium">{user?.full_name || '—'}</p></div>
    </div>
  );
}
